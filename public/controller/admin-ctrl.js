angular.module('timesheet')
    .factory('adminService', function ($http) {
        return {
            ...
            getProjectMembers: (project_id) => {
                return new Promise(function (resolve, reject) {
                    $http({
                        ...
                        if (response.data.message.constructor == String) resolve(new Array());
            ...
            assignApprover: (project_id, approver_id, employee_id) => {
                return new Promise(function (resolve, reject) {
                    $http({
                        method: 'POST',
    ...
    .controller('projectManageCtrl', function ($scope, $window, $http, ngDialog, adminService) {
        ...
        Promise.all([adminService.getAllUsers(session.token), adminService.getAllProjects(session.token)]).then(value => updateProject(value));
        ...
        $scope.addNewProject = () => {
            if ($scope.project_name == undefined || $scope.leader_id == undefined || $scope.project_name == '') alert('Bạn phải nhập tên dự án và tên trưởng dự án')
                ...
                $http({
                    ...
                    console.log(response.data.message);
                    adminService.getAllProjects(session.token).then(projects => updateProject([$scope.users, projects]));
                }, function errorCallback(response) {
                    console.log(response.data.message)
                })
            }
        ...
        $scope.showDetail = (project) => {
            ...
            dialog.closePromise.then(function () {
                adminService.getAllProjects(session.token).then(projects => $scope.$evalAsync(updateProject([$scope.users, projects])));
        ...
        $scope.saveChange = () => {
            ...
            $scope.new_project_employees.forEach((e) => {
                ...
                $http({
                    ...
                    console.log(response.data.message)
                    adminService.assignApprover(project_id, leader_id, employee_id)
                        .then(function (value) {
                            console.log(value);
                        }, function (problem) {
                            console.log(problem);
    ...
    .controller('timesheetManageCtrl', function ($scope, $window, $http, $timeout, ngDialog, adminService) {
        ...
        Promise.all([adminService.getAllUsers(session.token), adminService.getAllProjects(session.token), adminService.getAllTimesheets()])
            .then(value => {
        ...
        $scope.approve = (timesheet) => {
            ...
            dialog.closePromise.then(function (data) {
                adminService.getAllTimesheets().then(timesheets => updateData(_allUsers, _allProjects, timesheets));
            });
        }
        $scope.view = () => {
    ...
    .controller('timesheetDetailCtrl', function ($scope, $http) {
    ...
    .controller('approverManageCtrl', function ($scope, $window, $http, adminService) {
        ...
        Promise.all([adminService.getAllUsers(session.token), adminService.getAllProjects(session.token), getAllApprovers()])
            .then(value => {
                var [allUsers, allProjects, allApprovers] = value;
                updateDatas(allUsers, allProjects, allApprovers);
            });
        $scope.$watch('project_id', () => {
            adminService.getProjectMembers($scope.project_id)
                .then(function (value) {
                    $scope.$evalAsync(() => {
                        $scope.project_users = value;
                    })
                }, function (problem) {
                    console.log(problem);
                });
        ...
        $scope.addNewApprover = () => {
            if ($scope.project_id == null || $scope.approver_id == null || $scope.employee_id == null)
                ...
                adminService.assignApprover($scope.project_id, $scope.approver_id, $scope.employee_id)
                    .then(function (value) {
                        console.log(value);
